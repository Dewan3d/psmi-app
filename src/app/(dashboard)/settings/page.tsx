    ...
    <div className="bg-white rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] overflow-hidden border border-slate-100">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        {isAdmin && (
          <button onClick={() => setShowForm(!showForm)}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors cursor-pointer">
      ...
      <div className="divide-y divide-slate-100">
        ...
        {paginatedProducts.map((p) => (
          <div key={p.sku} className="px-5 py-3.5 hover:bg-slate-50/50 transition-colors">
            {editingSku === p.sku ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-medium text-slate-500 bg-slate-100 rounded-lg px-2.5 py-1">Alert at {p.low_stock_threshold}</span>
                {isAdmin && (
                  <div className="flex items-center gap-1">
                    <button onClick={() => handleEdit(p)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors cursor-pointer" title="Edit SKU & Description"><Edit2 className="w-3.5 h-3.5" /></button>
                    {deleteConfirm === p.sku ? (
                      <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => setDeleteConfirm(p.sku)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors cursor-pointer" title="Delete SKU"><Trash2 className="w-3.5 h-3.5" /></button>
                    )}
                  </div>
                )}
              </div>
